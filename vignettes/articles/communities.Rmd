...
communities_colors <- stats::setNames(
  grDevices::palette.colors(n = length(levels(communities$community))),
  levels(communities$community)
)

raster_colors <- grDevices::hcl.colors(
  n = 100,
  palette = "Zissou 1"
)
```
...
These maps and projections look cute and all that, but they don't represent reality in any meaninful way. Many processes that govern vegetation dynamics are not captured here, including species dispersal limitations, biotic interactions, and community resilience or adaptation. On top of that, the nature of random forest prevents it from predicting increased suitability even under improving conditions, unlike parametric models such as GLMs or GAMs.

An interesting exercise here is repeating these projections using different models and seeing what happens. If that doesn't break your faith in ability of distribution models to predict distribution changes under climate change, nothing will!
