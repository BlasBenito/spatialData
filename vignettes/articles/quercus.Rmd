---
title: "European oak distribution: multi-species niche modelling with decision trees"
output:
...
Quercus oaks are keystone species that shape European forest composition, yet distinguishing their climatic niches is challenging given overlapping ranges and correlated predictors. The `quercus` dataset is designed for exactly that problem.
...
Now let's put `quercus` to work.

In this example we use `quercus` to identify the environmental conditions that distinguish oak species using multicollinearity filtering and a decision tree.
...
Assigning predictions back to the spatial data reveals where each species is most likely to occur.
...
With the model trained, we can project it onto raster predictors.
...
That's all for now — happy oak modelling!
