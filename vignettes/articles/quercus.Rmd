...
This dataset was built to support a range of analytical approaches:

**Species Distribution Modeling**

- *Multi-class classification* - Predict which oak species is most likely at a location.
- *Binary presence/absence* - Filter to a single species for logistic regression or SDM.
- *Niche modeling* - Characterize environmental niches of co-occurring species.

**Statistical Methods**

- *Multicollinearity selection* - 31 numeric predictors with correlated groups.
- *Decision trees* - Classify species based on environmental thresholds.
- *Random forests / gradient boosting* - All-numeric predictor set.

**Spatial Analysis**

- *Spatial cross-validation* - Stratify by species or geographic region.
- *Niche overlap analysis* - Compare environmental ranges among species.
...
quercus |>
  sf::st_drop_geometry() |>
  dplyr::select(
    dplyr::all_of(quercus_predictors)
  ) |>
  head(n = 5) |>
  dplyr::glimpse()
```
...
Many predictors in this dataset are correlated (e.g., `bio5`, `bio6`, `bio7` are all derived from temperature extremes). The `collinear::collinear()` function selects a non-redundant subset that maximizes predictive power for the response variable.
...
Assigning tree predictions back to the spatial data reveals the geographic pattern of predicted species:
...
The decision tree also assigns a class probability to each observation for every species. The map below shows the fitted probability of *Quercus petraea* across Europe.
...
quercus$prob_petraea <- round(x = probs[, "Quercus petraea"], digits = 3)

mapview::mapview(
  quercus,
  zcol = "prob_petraea",
  layer.name = "P(Q. petraea)",
  col.regions = viridis::viridis(n = length(unique(quercus$prob_petraea)))
)
```

The decision tree provides interpretable rules for species separation, but more flexible methods like random forests or gradient boosting may achieve higher accuracy. The dataset also supports binary modeling by filtering to a single species and its absence points.
