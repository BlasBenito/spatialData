...
- *Ordinal regression* - `vi_factor` (ordered: very_low → very_high).
...
This brief example shows how `vi` can be used to train random forest model. It is 
...
The code below assigns to each observation the name of the predictor matters most for that specific location's prediction to reveal geographic patterns in variable relevance.
...
mapview::mapview(
  ...
  col.regions = viridis::turbo(n = length(unique(vi$local_importance)))
)
```
...
The usage example above is only one of the many modelling approaches that can be applied to `vi`. 

The dataset also supports binary classification (`vi_binomial`), count models (`vi_counts`), and ordinal regression (`vi_factor`). Categorical predictors like `continent`, `biogeo_biome`, and `koppen_zone` enable grouped and stratified analyses. The nature of the different predictors 
