...
The Normalized Difference Vegetation Index (NDVI) is one of the most widely used proxies for vegetation activity, yet its relationship with climate, soil, and human impact varies enormously across biomes. The `vi` dataset captures that variation at global scale, with ~10 000 records spanning 6 continents and 12 biomes, and a large set of numeric and categorical predictors (see `vi_predictors`) with consistent naming patterns (e.g., `temperature_*`, `rainfall_*`, `swi_*`).
...
Notice that the spatial distribution is irregular — records are limited to vegetated areas (`vi$vi_numeric > 0`).
...
The `vi_predictors` vector contains both numeric and categorical variables. Let's start by narrowing the predictor pool to numeric variables. In this example we focus on the numeric ones, which can be extracted using `collinear::identify_numeric_variables()`.
...
Now let's train a model and see what drives NDVI globally.
...
Here we assign to each observation the predictor that matters most for its specific location, revealing geographic patterns in variable relevance.
...
With the model trained, we can explore how NDVI responds to the most important predictors.
...
The usage example above is only one of the many modelling approaches that can be applied to `vi`. This workflow walked through random forest regression, permutation-based importance, local variable relevance, and response curves and surfaces — all with a single global dataset.
