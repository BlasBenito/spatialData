...
At this point we could evalate this model via spatial cross-validation, assess variable importance, and even check the response curves to understand how it's working. However, these steps are omitted here to keep the article light of content and computationally efficient.

We predict the fitted binary model to every cell of the raster `andalusia_env` to produce a binary raster.
...
Cells with value 1 in the map below represent environmental conditions where it is more likely to find a presence than a background point.
...
mapview(
  ...
  mapview(
    ...
    col.regions = c(colors[25], colors[75]),
...
We got a pretty one here, didn't we?
