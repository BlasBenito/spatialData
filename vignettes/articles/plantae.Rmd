...
You can download the Rmarkdown notebook used to render this article [here](https://github.com/BlasBenito/spatialData/blob/main/vignettes/articles/plantae.Rmd). To download the file, use the button *Download raw file* on the upper-right hand of the `Code` panel.
...
colors_richness <- viridis::rocket(
    n = 10,
    direction = -1
  )
```
...
The dataset is an `sf` POINT data frame (EPSG 4326) with `r nrow(plantae)` rows and `r ncol(plantae)` columns, and no missing data. The first 10 records and all columns but `geometry` are shown below.
...
Several significant ecoregions are missing from `plantae`:
...
- Sechura desert 
- Isthmian-Pacific moist forests (several patches missing)
...
There are no plans to bring them to the dataset.
...
The `plantae` dataset has `r length(plantae_predictors)` predictors with varying levels of redundancy. To manage multicollinearity in `plantae` we use [`collinear::collinear()`](https://blasbenito.github.io/collinear/reference/collinear.html). The function prioritizes the predictors in `preference_order` as requested by the user, while the remaining ones are ranked by the R-squared of univariate random forest models fitted against the response. The predictors are then evaluated one by one for selection following this ranking: if the variance inflation factor of the candidate predictor against the selected predictors is lower than 2.5, the predictor is added to the selection. Otherwise it is ignored, and the next predictor is evaluated.
...
Let's first train a non-spatial model to confirm that spatial structure is a problem. The code below trains a non-spatial random forest model via [`spatialRF::rf()`](https://blasbenito.github.io/spatialRF/reference/rf.html) and assessing the spatial autocorrelation of the residuals.
...
The function [`spatialRF::rf_spatial()`](https://blasbenito.github.io/spatialRF/reference/rf_spatial.html) takes the model above, adds *spatial predictors* (namely *Moran's Eigenvector Maps* or MEMs) to the training data. 
...
The spatial model has a performance very similar to the non-spatial one (RMSE = `r round(plantae_rf_spatial$performance$rmse.oob, digits = 2)`; R-squared =  `r round(plantae_rf_spatial$performance$r.squared.oob, digits = 2)`). 
...
+ `geo_perimeter_km` and `geo_area_km2` represent the importance of the contact surface with other ecoregions and the total ecoregion size.
